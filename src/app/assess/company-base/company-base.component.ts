...
@Component({
  selector: 'app-assess-company-base',
  templateUrl: './company-base.component.html',
  styleUrls: ['./company-base.component.css'],
  providers: [ HttpClientService ]
})
export class AssessCompanyBaseComponent implements OnInit {
  ...
  environmentalRequiredValidator=(control: FormControl): { [ s: string ]: boolean } => {
	...
	save():void{
		...
		_this.httpClientService.post("ecp","riskAssassment.updateCompanyBasicInfo",submitData)
    ...
    _this.httpClientService.post("ecp","riskAssassment.updateCompanyBasicInfo",submitData)
      .then(function(data:any){
        _this.router.navigateByUrl("assess/company/natural/"+_this.id);
      },function(err){
        _this.message.error(err);
      });
  }
	...
	initData(){
		...
		_this.httpClientService.post("ecp","riskAssassment.getCompanyBasicInfo",{projectId:_this.id,userId:1})
		.then(function(data:any){
