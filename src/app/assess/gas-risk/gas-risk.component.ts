...
@Component({
  selector: 'app-assess-gas-risk',
  templateUrl: './gas-risk.component.html',
  styleUrls: ['./gas-risk.component.css'],
	providers: [HttpClientService]
})
export class GasRiskComponent implements OnInit {
  ...
  save(){
  	...
  	let submitData:any=new Object();
		submitData.gasRadioID=_this.tableData.gasRadioID;
  	...
  	_this.httpClientService.post("ecp","riskAssassment.updateGasRatio",submitData)
  ...
  next(){
  	...
  	if(_this.isPassValid){
  		...
  		_this.httpClientService.post("ecp","riskAssassment.updateGasRatio",submitData)
  		.then(function(data:any){
  			_this.router.navigateByUrl("assess/calculate/waterRisk/"+_this.id);
  		},function(err){
  			_this.message.error(err);
  		});
  	}
  ...
  initData(){
  	let _this = this;
  	_this.httpClientService.post("ecp", "riskAssassment.getGasRatio", {projectId: _this.id, userId: 1})
  	  .then(function (data: any) {
