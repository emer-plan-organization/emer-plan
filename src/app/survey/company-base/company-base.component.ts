...
@Component({
  selector: 'app-survey-company-base',
  templateUrl: './company-base.component.html',
  styleUrls: ['./company-base.component.css'],
  providers: [ HttpClientService ]
})
export class SurveyCompanyBaseComponent implements OnInit {
	...
	environmentalRequiredValidator=(control: FormControl): { [ s: string ]: boolean } => {
		if(this.validateForm && this.validateForm.controls){
			if(this.validateForm.controls.EnvironmentalAccidents.value==null){
				if(!control.value || control.value==null || control.value.toString()==""){
					return { confirm: true, error: true };
				}
	...
	save():void{
		...
		_this.httpClientService.post("ecp","ecpContingencyResource.updateCompanyBasicInfo",submitData)
	...
	next():void{
		...
		if(this.validateForm.status=="VALID"){
			...
			.then(function(data:any){
				//返回成功则更改表状态
				//post("hzx", "hzxCommon.updatebasetablestate", {evaluateID:_this.$route.query.evaluateId,itemCode:"P0010101",caozuoState:2}).then(function(data){
				_this.router.navigateByUrl("survey/emergency/org/"+_this.id);
				/*_this.httpClientService.post("hzx","hzxCommon.updatebasetablestate",{evaluateID:_this.id,itemCode:"P0010101",caozuoState:2})
				.then(function(data:any){
					_this.router.navigateByUrl("survey/environmen/org/"+this.id);
				},function(err){
					//_this.$Message.error(err);
	...
	initData(){
		...
		_this.httpClientService.post("ecp","ecpContingencyResource.getCompanyBasicInfo",{projectId:_this.id,userId:1})
		.then(function(data:any){
	...
	accidentsChange(value){
		if(value==0||value=="0"){
			this.company.Fire=null;
			this.company.FireNumber=null;
			this.company.Explosion=null;
			this.company.ExplosionNumber=null;
			this.company.Reveal=null;
			this.company.RevealNumber=null;
		}
	}
